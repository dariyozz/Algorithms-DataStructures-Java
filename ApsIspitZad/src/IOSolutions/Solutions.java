...
public class Solutions {
    public static void alterTeams(SLL devTeam, SLL qaTeam) {
        SLLNode tmp = qaTeam.first;
        SLLNode qaNajmlad = tmp;
        while (tmp != null) {
            if (qaNajmlad.age >= tmp.age) {
                qaNajmlad = tmp;
            }
            tmp = tmp.succ;
        }
        ...
        while (tmp != null) {
            if (cnt == mid) {
                SLLNode newCard = new SLLNode(qaNajmlad.id, qaNajmlad.age, tmp.succ);
                tmp.succ = newCard;
                break;
            }
            cnt++;
