...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        //      findMostPowerfulCard
        SLLNode<Card> bestCard = firstSorcererCards.getFirst();
        SLLNode<Card> nextCard = firstSorcererCards.getFirst().succ;

        while (nextCard != null) {
            if ((nextCard.element.significance()) > (bestCard.element.significance())) {
                bestCard = nextCard;
            }
            nextCard = nextCard.succ;
        }
        firstSorcererCards.delete(bestCard);

        //insert in second deck middle
        int middle = secondSorcererCards.size()/2;
        SLLNode<Card> findMiddle = secondSorcererCards.getFirst();
        int cnt = 1;
        while (findMiddle != null){
            if(middle==cnt){
                SLLNode<Card> newCard = new SLLNode<Card>(bestCard.element,findMiddle.succ);
                findMiddle.succ=newCard;
                break;
            }
            findMiddle = findMiddle.succ;
            cnt++;
        }
    }
