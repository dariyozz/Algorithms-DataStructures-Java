...
public class ArrayMeanWordLength {
    ...
    public static String wordClosestToAverageLength(Array<String> arr) {
        ...
        for (int i = 0; i < arr.getSize(); i++) {
            sumLenghts += arr.get(i).length();
        }
        average = (double) sumLenghts / arr.getSize();
        String closestWord = arr.get(0);
        double closestDiffrence = Math.abs(closestWord.length() - average);
        for (int i = 1; i < arr.getSize(); i++) {
            String novWord = arr.get(i);
            double diff = Math.abs(novWord.length() - average);
            if (diff < closestDiffrence || (closestDiffrence == diff && novWord.length() > closestWord.length())) {
                closestWord = novWord;
                closestDiffrence = diff;
            }
        }
        return closestWord;
    }
    ...
    public static void main(String[] args) {
        ...
        int N = input.nextInt();
        Array<String> arr = new Array<String>(N);
        input.nextLine();
